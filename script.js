...
// FIX 4: updateLiveClock manages its own interval, no return value needed
function updateLiveClock(tzId, currentTemp = null) {
  ...
  // FIX 4: removed return — clockInterval is managed globally above
}
...
async function getVibe(city, originalQuery = null) {
  ...
  // FIX 7: disable search button while fetching
  const searchBtn = document.getElementById('searchBtn');
  if (searchBtn) { searchBtn.disabled = true; searchBtn.innerText = '불러오는 중...'; }

  // FIX 2: Korean fallback map checked BEFORE Nominatim lookup
  if (/[가-힣]/.test(city)) {
    const fallback = {
    ...
    // FIX 2: if in fallback, redirect immediately before Nominatim
    if (fallback[city]) {
      if (searchBtn) { searchBtn.disabled = false; searchBtn.innerText = '검색'; }
      return getVibe(fallback[city], originalQuery);
    }
  ...
  try {
    ...
    if (data.error) {
      // FIX 2: fallback map for non-Korean searches only reaches here now
      if (searchBtn) { searchBtn.disabled = false; searchBtn.innerText = '검색'; }
      return alert(`도시를 찾을 수 없어요 😢\n서울, 부산, 수원 같은 큰 도시명으로 검색해보세요!`);
    }
    ...
    // FIX 5: fallback to last hour, not first, to avoid returning a past hour
    ...
    if (T <= 10 && v >= 4.8) {
      ...
      // FIX 3: Heat Index should never read lower than actual temperature
    ...
    document.getElementById('runVal').innerText =
      (temp > 12 && temp < 25 && !isRain) ? "뛰기 딱 좋아요 🏃‍♀️" : "오늘은 조금 피하는 게 좋아요";
    document.getElementById('driveVal').innerText =
      (windSpeedKmh > 20 || isRain) ? "조심해서 운전해요 🚗" : "편하게 운전해도 괜찮아요";
    ...
    // FIX 4: just call, don't capture return value
    ...
    // FIX 7: always re-enable search button
    if (searchBtn) { searchBtn.disabled = false; searchBtn.innerText = '검색'; }
  }
}
...
function renderForecast(forecastDay) {
  ...
  if (!container) return;
  // FIX 6: build string first, set innerHTML once
  let html = '';
...
function renderHourly(allHours, localTimeStr) {
  ...
  if (!container) return;
  // FIX 1: compare as strings — same "YYYY-MM-DD HH:MM" format, no timezone parse issue
